 ...
 */
async function createConfluenceImageMacro(attachmentName, altText = '', options = {}) {
    const { pageId, confluenceClient, preferBlobUrl = true } = options;
    
    // Check if the attachment name has a size parameter
    const hasWidthParameter = attachmentName.includes("=") && attachmentName.split("=")[1].includes("x");
    let width = "";
    ...
    if (preferBlobUrl && pageId && confluenceClient) {
        try {
            const blobUrl = await getBlobUrl(confluenceClient, pageId, cleanAttachmentName);
            if (blobUrl) {
                // Include width parameter if present
                if (width) {
                    return `<ac:image${altText ? ` ac:alt="${altText}"` : ''}><ri:url ri:value="${blobUrl}" /><ac:parameter ac:name="width">${width}</ac:parameter></ac:image>`;
                } else {
                    return `<ac:image${altText ? ` ac:alt="${altText}"` : ''}><ri:url ri:value="${blobUrl}" /></ac:image>`;
                }
            }
        } catch (error) {
            console.warn(`Error getting blob URL for ${cleanAttachmentName}:`, error);
        }
    }
    
    // Fallback to standard attachment reference with width if specified
    if (width) {
        return `<ac:image${altText ? ` ac:alt="${altText}"` : ''}><ri:attachment ri:filename="${cleanAttachmentName}" /><ac:parameter ac:name="width">${width}</ac:parameter></ac:image>`;
    } else {
        return `<ac:image${altText ? ` ac:alt="${altText}"` : ''}><ri:attachment ri:filename="${cleanAttachmentName}" /></ac:image>`;
    }
}
 ...
 */
async function convertImages(content, attachmentMappings, confluenceClient, pageId) {
    ...
    processedContent = processedContent.replace(/!([\w.-]+\.(png|jpg|jpeg|gif|svg))/gi, (match, filename) => {
        console.log(`Processing simple image reference: ${match}`);
    ...
    async function processImageMatch(match, alt, src) {
        console.log(`Processing standard Markdown image: ${match}`);
        ...
        if (src.startsWith('/.attachments/') || src.startsWith('../') || src.includes('/attachments/')) {
            const attachmentName = decodeURIComponent(src.split('/').pop());
            console.log(`Extracted attachment name: ${attachmentName}`);
            
            // Add the size info if present
            const attachmentNameWithSize = sizeInfo ? `${attachmentName}=${sizeInfo}` : attachmentName;
            ...
            if (attachmentKey && attachmentMappings[attachmentKey]) {
                ...
                if (mimeType === 'application/pdf' ||
                    ...
                    return await createConfluenceImageMacro(attachmentNameWithSize, alt || attachmentName, { confluenceClient, pageId });
                } else {
                    // For other file types, use a download link macro
                    return `<ac:link><ri:attachment ri:filename="${attachmentName}" /><ac:plain-text-link-body><![CDATA[${alt || attachmentName}]]></ac:plain-text-link-body></ac:link>`;
                }
            } else {
            ...
            console.log(`Converting relative attachment path: ${src} -> ${attachmentNameWithSize}`);
            return await createConfluenceImageMacro(attachmentNameWithSize, alt || attachmentName, { confluenceClient, pageId });
        }
    }
    ...
    while ((wikiMatch = wikiImageRegex.exec(processedContent)) !== null) {
        ...
        console.log(`Processing wiki-style image reference: ${fullMatch}`);
        
        if (!imagePath) continue;
        
        // Extract filename from path
        const attachmentName = decodeURIComponent(imagePath.split('/').pop());
        console.log(`Extracted wiki-style attachment name: ${attachmentName}`);
        
        // Use the utility function
        const replacement = await createConfluenceImageMacro(attachmentName, attachmentName, { confluenceClient, pageId });
        processedContent = processedContent.replace(fullMatch, replacement);
    }
    ...
    while ((htmlMatch = htmlImgRegex.exec(processedContent)) !== null) {
        ...
        console.log(`Processing HTML img tag: ${fullMatch.substring(0, 50)}...`);
        
        if (src.startsWith('http://') || src.startsWith('https://')) {
            // For external URLs, use Confluence URL image macro
            processedContent = processedContent.replace(fullMatch, `<ac:image><ri:url ri:value="${src}" /></ac:image>`);
        } else if (src.includes('.attachments/') || src.includes('/attachments/') || src.startsWith('../')) {
            // Handle relative paths to attachments
            const attachmentName = decodeURIComponent(src.split('/').pop());
            console.log(`Converting HTML img src to attachment: ${src} -> ${attachmentName}`);
            ...
            // Use the utility function
            const replacement = await createConfluenceImageMacro(attachmentName, altText, { confluenceClient, pageId });
            processedContent = processedContent.replace(fullMatch, replacement);
        }
    }
...
function restoreCodeBlocks(content, codeBlocks) {
    ...
    if (typeof content !== 'string') {
        console.warn(`restoreCodeBlocks received non-string content: ${typeof content}`);
        try {
            content = String(content || '');
        } catch (error) {
            console.error('Failed to convert content to string in restoreCodeBlocks:', error);
            return '';
        }
    }
    ...
    if (!codeBlocks || typeof codeBlocks.forEach !== 'function') {
        console.warn(`restoreCodeBlocks received invalid codeBlocks: ${typeof codeBlocks}`);
    ...
    try {
        ...
        codeBlocks.forEach((block, id) => {
            if (!block || typeof block !== 'object') {
                console.warn(`Invalid code block found for ID ${id}`);
            ...
            try {
                ...
                restoredContent = restoredContent.replace(id, `
...
function processCodeBlocks(content) {
    // Input validation
    if (typeof content !== 'string') {
        console.warn(`processCodeBlocks received non-string content: ${typeof content}`);
...
function cleanupContent(content) {
    ...
    if (typeof content !== 'string') {
        console.warn(`cleanupContent received non-string content: ${typeof content}`);
        // Return a safe default if content is not a string
        return typeof content === 'undefined' ? '<p>No content</p>' : String(content);
    }
 ...
 */
function processConfluenceLinks(content, attachmentMappings, pageId) {
    // Input validation
    if (typeof content !== 'string') {
        console.warn(`processConfluenceLinks received non-string content: ${typeof content}`);
        try {
            content = String(content || '');
        } catch (error) {
            console.error('Failed to convert content to string:', error);
    ...
    try {
        ...
        let processedContent = content.replace(/\[\[([^\]]+)]]/g, (match, pageName) => {
            console.log(`Processing wiki-style link: ${match}`);
            // Create a link to the page
            return `[${pageName}](/wiki/spaces/SD/pages/${pageName.replace(/\s+/g, '-')})`;
        });
 ...
 */
async function convertMarkdownToConfluenceHtml(markdown, attachmentMappings, pagePath, confluenceClient, pageId) {
    try {
        console.log(`Starting conversion of markdown to HTML for page at ${pagePath}`);
        
        // Check if markdown is provided
        if (!markdown) {
            console.warn(`No markdown content provided for page at ${pagePath}`);
        ...
        if (markdown instanceof Promise) {
            try {
                console.log(`Resolving promise for content of page at ${pagePath}`);
                markdownContent = await markdown;
            } catch (error) {
                console.error(`Error resolving content promise for page at ${pagePath}:`, error);
        ...
        if (typeof markdownContent !== 'string') {
            console.warn(`Expected string content for page at ${pagePath}, got ${typeof markdownContent}`);
        ...
        try {
            ...
            let processedContent = processConfluenceLinks(content, attachmentMappings, pageId);
            
            // Process images - this was missing!
            processedContent = await convertImages(processedContent, attachmentMappings, confluenceClient, pageId);
            ...
            console.log(`Successfully converted markdown to HTML for page at ${pagePath}`);
            return html;
        } catch (error) {
            console.error(`Error converting markdown to HTML for page at ${pagePath}:`, error);
            return `<p>Error converting content: ${error.message}</p>
                  <pre>${markdownContent ? markdownContent.substring(0, 1000) + '...' : 'No content'}</pre>`;
        }
    } catch (error) {
        console.error(`Error in convertMarkdownToConfluenceHtml for page at ${pagePath}:`, error);
...
function convertMarkdown(content) {
    // Input validation
    if (typeof content !== 'string') {
        console.warn(`convertMarkdown received non-string content: ${typeof content}`);
        try {
            content = String(content || '');
        } catch (error) {
            console.error('Failed to convert content to string:', error);
...
function convertAttachmentLinksToImageMacros(html) {
    if (typeof html !== 'string') {
        console.warn(`convertAttachmentLinksToImageMacros received non-string content: ${typeof html}`);
        return html || '';
    }
    ...
    try {
        ...
        return html.replace(/<a[^>]*href=["'](\/\.attachments\/[^"']+\.(png|jpg|jpeg|gif|svg)[^"']*)["'][^>]*>(.*?)<\/a>/gi, (match, path, ext, text) => {
            console.log(`Converting attachment link to image macro: ${path}`);
            ...
            const filenameWithSize = sizeParameter ? `${filename}=${sizeParameter}` : filename;
