...
async function createOrUpdatePage(page, confluenceClient, confluenceConfig, spaceKey, parentPageId, attachmentMappings) {
    try {
      ...
      if (!pageContent) {
        console.warn(`No content found for page ${originalTitle}, using empty content`);
        pageContent = '';
      }
      ...
      if (pageContent instanceof Promise) {
        try {
          ...
          console.error(`Error resolving content promise for page ${originalTitle}:`, error);
          pageContent = '';
        }
      }
  ...
  async function deletePagesUnderParent(confluenceClient, parentPageId) {
    try {
      ...
      if (childPages && childPages.length > 0) {
        console.log(`Found ${childPages.length} child pages under parent page ID: ${parentPageId}`);
        
        // Recursively delete child pages
        for (const childPage of childPages) {
          console.log(`Deleting child page: ${childPage.title} (ID: ${childPage.id})`);
      ...
      console.error(`Error deleting pages under parent page ID ${parentPageId}:`, error);
      throw error;
    }
  }
  /**
  ...
  async function updatePage(confluenceClient, pageId, title, content, spaceKey) {
    try {
      ...
      const currentPage = await confluenceClient.getPageById(pageId, { expand: 'version' });
